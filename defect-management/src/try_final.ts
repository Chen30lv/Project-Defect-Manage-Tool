...
export class SideBarBeeHivetest implements vscode.TreeDataProvider<SideBarEntryItem> {
    constructor(private workspaceRoot?: string) {}
      ...
      if (element) {
        ...
        for (let index = 0; index < scripts.length; index++) {
          ...
          item.command = {
            ...
            title: scripts[index].script,
            arguments: [scripts[index].inf], // Arguments passed to the command
          };
          childrenList[index] = item;
        }
        return childrenList;
...
module.exports = function (context: vscode.ExtensionContext) {
  ...
  vscode.commands.registerCommand(
    ...
    () => {
       // xinjia
       vscode.window.showInputBox({ prompt: 'Enter your message' }).then(message => { 
